...
TRADE_RESULTS_TIME_ZONE = ZoneInfo('America/New_York')
TRADE_RESULTS_RESET_HOUR = 12
...
def _current_trade_results_window(now=None):
    """Return the current accounting window for trade results.

    The daily result resets every day at 12:00 PM in Manhattan/New York City.
    Example: if it is 10:00 AM in New York, the active window started
    yesterday at 12:00 PM. If it is 2:00 PM, it started today at 12:00 PM.
    """
    current = now or timezone.now()
    local_now = current.astimezone(TRADE_RESULTS_TIME_ZONE)
    start_local = local_now.replace(
        hour=TRADE_RESULTS_RESET_HOUR,
        minute=0,
        second=0,
        microsecond=0,
    )
...
def _daily_trade_result_for(user):
    start_local, end_local = _current_trade_results_window()

    base_queryset = TradeOrder.objects.filter(
        user=user,
        status=TradeOrder.STATUS_CLOSED,
        closed_at__isnull=False,
        closed_at__gte=start_local,
        closed_at__lt=end_local,
    )

    total = base_queryset.aggregate(total=Sum('profit_loss')).get('total') or Decimal('0.00')

    latest_order = base_queryset.order_by('-closed_at', '-id').first()
    latest_result = latest_order.profit_loss if latest_order else Decimal('0.00')

    return (
        total.quantize(Decimal('0.01')),
        latest_result.quantize(Decimal('0.01')),
        start_local,
        end_local,
    )


def _trade_results_payload(user):
    today_result, latest_result, start_local, end_local = _daily_trade_result_for(user)

    return {
        # Account Results = uniquement le résultat du dernier trade fermé.
        'account_results': str(latest_result),
        'last_trade_result': str(latest_result),
        # Today = total cumulé des gains/pertes depuis midi New York.
        'today': str(today_result),
...
def _user_payload(user):
    ...
    return {
        ...
        'trade_results': _trade_results_payload(user),
    }
...
def app_admin_set_vip(request):
    ...
    return JsonResponse({
        ...
        'message': 'VIP badge updated successfully.',
    })
