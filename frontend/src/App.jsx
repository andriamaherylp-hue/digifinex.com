...
function MyPage({ go, overlay, user }) {
  const tradeResults = user?.trade_results || {}
  const accountResults = tradeResults.account_results ?? tradeResults.last_trade_result ?? 0
  const todayResults = tradeResults.today ?? 0
  const accountResultsClass = Number(accountResults) >= 0 ? 'positive' : 'negative'
  const todayResultsClass = Number(todayResults) >= 0 ? 'positive' : 'negative'
    ...
    <AppFrame page="my" go={go} className="my-page">
      <section className="profile-top">
        <div className="profile-name">
          ...
          <p>{user?.username || 'client'} <span>{user?.is_vip ? 'VIP' : 'Regular'}</span></p>
          <div className="profile-meta-line">
            <small><MdOutlineVerifiedUser /> Credit Score:{user?.credit_score || 100}</small>
            {user?.is_vip && <img className="vip-badge-image" src={vipBadgeImage} alt="VIP" />}
          </div>
        </div>
      ...
      <section className="asset-card">
        ...
        <div className={`asset-result ${accountResultsClass}`}><small>Account Results</small><b>{signedMoney(accountResults)}</b></div>
        <div className={`asset-result ${todayResultsClass}`}><small>Today</small><b>{signedMoney(todayResults)}</b></div>
      </section>
...
function AdminPage({ back, user }) {
  ...
  const deleteClient = async (client) => {
    ...
    try {
      ...
      if (String(vipClientId) === String(client.id)) setVipClientId('')
      loadAdmin()
    } catch (error) {
      toast.error(error.message || M.serverError)
    }
  ...
  const updateVipBadge = async (isVip) => {
    if (!vipClientId) return toast.error('Please select a client account first.')

    try {
      await apiRequest('/api/app/admin/set-vip/', {
        method: 'POST',
        body: JSON.stringify({ user_id: vipClientId, is_vip: isVip }),
      })
      toast.success(isVip ? 'VIP badge assigned successfully' : 'VIP badge removed successfully')
      loadAdmin()
    } catch (error) {
      toast.error(error.message || M.serverError)
    }
  ...
  const selectedVipClient = clients.find((client) => String(client.id) === String(vipClientId))
  ...
  return (
    <div className="app-page admin-page">
      ...
      <section className="admin-grid">
        ...
        <div className="admin-card vip-admin-card">
          <h3>VIP badge management</h3>
          <label className="admin-field-label">Client account</label>
          <select value={vipClientId} onChange={(e) => { setVipClientId(e.target.value); setSelectedClient(e.target.value) }}>
            <option value="">Select client account</option>
            {clients.map((client) => (
              <option key={client.id} value={client.id}>
                {client.username} — {client.account_code} — {client.is_vip ? 'VIP' : 'Regular'}
              </option>
            ))}
          </select>
          {selectedVipClient && (
            <div className="admin-selected-account compact vip-selected-account">
              <div className="vip-selected-title">
                <strong>{selectedVipClient.username}</strong>
                {selectedVipClient.is_vip && <img className="vip-badge-small" src={vipBadgeImage} alt="VIP" />}
              </div>
              <small>{selectedVipClient.account_code} · Status: {selectedVipClient.is_vip ? 'VIP' : 'Regular'}</small>
            </div>
          )}
          <div className="vip-action-buttons">
            <button className="primary-button" disabled={!vipClientId || selectedVipClient?.is_vip} onClick={() => updateVipBadge(true)}>Assign VIP</button>
            <button className="vip-remove-button" disabled={!vipClientId || !selectedVipClient?.is_vip} onClick={() => updateVipBadge(false)}>Remove VIP</button>
          </div>
        </div>
      </section>
            ...
            <div key={client.id} className={String(selectedClient) === String(client.id) ? 'admin-client-row selected' : 'admin-client-row'}>
              <button
                ...
                onClick={() => {
                  ...
                  setVipClientId(String(client.id))
                }}
              >
                <span>{client.username}</span>
                <span>{client.account_code}{client.is_vip ? ' · VIP' : ''}</span>
      ...
      <section className="admin-card wide">
        ...
        {closedTrades.length ? closedTrades.map((item) => (
          <div className="admin-request trade-admin-row" key={item.id}>
            ...
            <small>
              Profit/Loss: ${money(item.profit_loss)} | Close price: {item.close_price || '-'} | {formatAdminHistoryDateTime(item.closed_at) || '-'}
            </small>
          </div>
